...
public class Interface extends JFrame {
	...
	public Interface() {
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			if ((missingAngles <= 1 && missingSides <= 2)||(missingSides <= 1 && missingAngles <= 2) || (missingSides == 0 && missingAngles <= 3)){ //Determines if the input were logical
				JOptionPane.showMessageDialog(contentPane, "Valid input!");
				pass = true;
			}else{
			...
			//TODO add code for guessing right triangles
			if (!pass){
				if (thirtySixtyTest(sides)){
					int spot = newLargest(sides);
					angles[spot] = 90;
					pass = true; //We'll let it slide if this is true
					JOptionPane.showMessageDialog(contentPane, "Assuming an angle in attempt to make it valid.");
				}
				
				if (fortyFiveTest(sides)){
					int spot = newLargest(sides);
					angles[spot] = 90;
					pass = true;//like above, now we have technichally made the input valid
					JOptionPane.showMessageDialog(contentPane, "Assuming an angle in attempt to make it valid.");
				}
			}
			
			//The two if statements above are not tested, but in effect, they just assume an angle if the sides have a certain condition
			...
			do{ //do while loop!
				...
				System.out.println("Missing Sides: "+missingSides+"\nMissing Angles: "+missingAngles);
				...
				if (missingAngles == 1){ //if we have all but one angle, find it through subtraction
					...
					System.out.println("A missing angle was found via angle subtraction.");
				}
				...
				if ((missingAngles <= 1) && (missingSides == 1)){ //Scenario: 1 angle, at least two sides
					...
					System.out.println("A missing side was found via the law of CoSines.");
					
				} 
				if (missingSides == 0){
					if (masterSet[1] == 0)
					{
						masterSet[1] = a(masterSet);
						missingAngles--;
					}
					if (masterSet[3] == 0)
					{
						masterSet[3] = b(masterSet);
						missingAngles--;
					}
					if (masterSet[5] == 0)
					{
						masterSet[5] = c(masterSet);
						missingAngles--;
					}
					System.out.println("A missing angle was found via law of CoSines");
				}
					...
					if (ratioFound){//when the ratio is found do...
						System.out.println("ratio is known");
						for (int i = 0; i<masterSet.length; i++){ //go through all slots
							Double NUM = (double) 42;  //idk, its a test variable
							if (masterSet[i] == 0){ //is the slot empty?
								if (i%2 == 0 && masterSet[i+1] != 0){//Sides
									NUM = SideCalc(masterSet[i+1], ratio[1], ratio[0]); //solve side
									System.out.println("found the side for value of master set "+ (i)+ "which was previously "+masterSet[i]);
									System.out.println(masterSet[i]+" was set to "+NUM);
									masterSet[i] = NUM;
									System.out.println("Missing side was found via law of sines.");
									break;//NOTE we break here so that only one thing is calculated per cycle! This way, we can fill in info in other ways!
								}else if (i%2 != 0 && masterSet[i-1] != 0){
									NUM = AngleCalc(masterSet[i-1], ratio[1], ratio[0]); //solve angle
									System.out.println("found the value for place "+ (i)+", which was previously "+masterSet[i]+" using the known side of "+masterSet[i-1]);
									System.out.println("spot "+i+" was set to "+NUM);
									System.out.println("Missing angle was found via law of sines.");
									masterSet[i] = NUM;
									break;//such as here, where its much better to just subtract angles from 180
								}
							}
						}
...
//				}
				...
				for (int i = 0; i < masterSet.length; i++){ //I don't know why this is still here, its not really used 
					if (masterSet[i] == 0){// but Im scared if I remove it something will break
						masterSetEmpty[i] = false;
					}else{
						masterSetEmpty[i] = true;
					}
				...
				if (count > 5){ //Trial/error code, it was here because we used a while loop, not a for loop, and sometimes it hung
					System.out.println("Got stuck! Spitting out info so far!");
					Error = "Error: Could not be calculated";
					break;
				}
...
//			angles[2] = masterSet[5];
			...
			if (masterSet[0] == masterSet[2] && masterSet[2] == masterSet[4]){ //Equilateral brute force (because of estimations being weird sometimes)
				masterSet[1] = 60;
				masterSet[3] = 60;
				masterSet[5] = 60; 
			}
			...
			String text = "This triangle is "+TRIangle[k(masterSet)]+"."
					+ "\nThis triangle is "+TriANGLE[i(masterSet)]+".\n"
					+ Error;
			JOptionPane.showMessageDialog(contentPane,text);
			System.out.println(text);
			text = "Side A: "+masterSet[0]+
					"\nAngle a: "+masterSet[1]+
					"\nSide B: "+masterSet[2]+
					"\nAngle b: "+masterSet[3]+
					"\nSide C: "+masterSet[4]+
		...
		JLabel label = new JLabel((new ImageIcon(getClass().getResource("/triangle.png"))));
	...
	public static double AngleCalc(double knownSide, double ratioAngle, double ratioSide){
		double angle = Math.asin((knownSide/ratioSide)*Math.sin(ratioAngle));
		return Math.abs(Math.toDegrees(angle));
			//returns angle, which is a stand in for the angle being searched
	}
	
	public static boolean thirtySixtyTest(double[] sides){
		int counta = 0, countb = 0;
		boolean ans = false;
		for (int i = 0; i<sides.length;i++){
			for (int j = 0; j<sides.length;j++){
				if ((sides[j]/Math.sqrt(3) == sides[i])){
					counta++;
				}else if (sides[j]/2 == sides[i]){
					countb++;
				}
			}
		}
	...
	public static boolean fortyFiveTest(double[] sides){
		int count = 0;
		boolean ans = false;
		for (int i = 0; i<sides.length;i++){
			for (int j = 0; j<sides.length;j++){
    ...
    public static String [] TRIangle = {"Scalene", "Equilateral","Isoseles", "Not a Triangle"};
    ...
    //ISaiah's code ends here
	
    public static int newLargest(double[] numbers){
		...
		if (S == c){
			...
			return 42;//will never happen but hey
		}
