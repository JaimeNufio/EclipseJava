...
public class Grades2 {
	...
	public static void main(String[] args) {
		...
		String  Name, header, finalAvg = null;
		...
		System.out.printf("This program will average grades for %s.\nType in a negative value when you are done.\n",Name);
		header = "\nName:\tGrade:\tAssignment:\tAverage:\tLetter Grade:\n";
		...
		for (int i = 0; i <=count-1; i++){
			if ((count-1) == i){
				finalAvg = (int)((gradeSum)/(count))+"";
			}
			System.out.printf("%s\t%d\t%s\t%s\t%s\n",Name,(int)grades[i],(i+1)+position(i+1)+" assignment",finalAvg,letterGrade((int)(grades[i])));
		}
